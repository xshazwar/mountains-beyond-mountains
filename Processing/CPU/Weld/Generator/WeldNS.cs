...
namespace xshazwar.processing.cpu.weld{
    ...
    public struct WeldNS: IGenerateWelds {
        public int Resolution {get; set;}
        public int MarginWidth {get; set;}
        public WeldSide side {get; set;}
        public int JobLength => Resolution;
        public int[] cRange => new int[] {0, 2 * MarginWidth * Resolution, Resolution};
        public int tOff => side == WeldSide.N ? 0 : Resolution * Resolution - (Resolution * 2 * MarginWidth); 
        public int sOff => side == WeldSide.S ? 0 : Resolution * Resolution - (Resolution * 2 * MarginWidth); 

        public void Execute(int i, NativeSlice<float> source, NativeSlice<float> target){
            for (int c = cRange[0]; c < cRange[1]; c += cRange[2]){
                target[ i + c + tOff] = source[i + c + sOff];
            }
            // W:
                // i => (margin, res - margin)
                // c => 0; 2 * margin; +=1
